...
import { EXPERIMENTAL_STRUCTURE_SCENE_SCALE } from './experimental-scene-scale.js';

const SCALE = EXPERIMENTAL_STRUCTURE_SCENE_SCALE;
...
function resolveContentRoot(context) {
  return context?.contentRoot ?? context?.scene ?? null;
}

function requestRender(context) {
  if (typeof context?.requestRender === 'function') {
    context.requestRender('experimental-structure-layers:h-alpha');
    return;
  }
  if (typeof context?.renderOnce === 'function') {
    context.renderOnce();
    return;
  }
  if (typeof context?.runtime?.renderOnce === 'function') {
...
export function createHaTiledVolumeLayer(options = {}) {
  ...
  const api = {
    ...
    async attach(context) {
      ...
      if (!contentRoot || typeof contentRoot.add !== 'function') {
        throw new Error('H-alpha tiled volume layer attach() requires context.contentRoot or context.scene.');
      }
    ...
    dispose(context) {
      ...
      group.clear();
      resolveContentRoot(context)?.remove?.(group);
      display = null;
      service = null;
      notifyStats();
    },
